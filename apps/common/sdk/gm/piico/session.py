...
class Session(SM2Mixin, SM3Mixin, SM4Mixin):
    ...
    def verify_sign_ecc(self, alg_id, public_key, raw_data, sign_data):
        ...
        pk = ECCrefPublicKey(c_uint(0x100), (c_ubyte * len(k1))(*k1), (c_ubyte * len(k2))(*k2))
        ...
        ret = self._driver.SDF_ExternalVerify_ECC(self._session, c_int(alg_id), pointer(pk), plain_text,
                                                  c_int(len(plain_text)), pointer(signature))
        if ret != 0:
